...
//                        self.usersYouOwe.append(expense)
    ...
    func deleteUser(currentEmail: String, userEmail:String, completion: @escaping (Bool) -> Void) {
...
//            let userEmail = currentUser.email ?? ""
        ...
        usersCollection.whereField("email", isEqualTo: userEmail.prefix(1).capitalized + userEmail.dropFirst()).getDocuments { snapshot, error in
                if let error = error {
                    print("Error getting user document: \(error)")
                    // Handle the error
                    completion(false)
